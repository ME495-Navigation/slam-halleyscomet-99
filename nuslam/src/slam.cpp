...
/// measurements, while the Blue robot uses pure odometry anchored to 'nusim/world'.
class SlamNode : public rclcpp::Node
{
public:
  /// \brief Constructor for the SlamNode.
  /// \details Initializes parameters, kinematic models, and ROS interfaces.
  SlamNode()
  : Node("slam"), ekf_(20)
  {
    ...
    wall_sub_ = create_subscription<visualization_msgs::msg::MarkerArray>(
      "/nusimulator/real_walls", 10, std::bind(&SlamNode::wall_callback, this, std::placeholders::_1));
    ...
    // Static frame assignments for Path messages
    slam_path_msg_.header.frame_id = map_id_;
    odom_path_msg_.header.frame_id = "nusim/world"; 
    
    RCLCPP_INFO(get_logger(), "SLAM initialized. Static Frame: nusim/world");
  }

private:
  /// \brief Callback for encoder data (Prediction Step).
  /// \details Updates SLAM prediction and pure odometry accumulation.
  /// \param msg - The shared pointer to the incoming JointState message.
  ...
  /// \brief Callback for EKF Correction Step.
  /// \param msg - Landmark measurements relative to the robot.
  ...
  /// \brief Stores simulator walls for visualization.
  /// \param msg - Walls from the nusimulator.
  void wall_callback(const visualization_msgs::msg::MarkerArray::SharedPtr msg)
  {
    slam_walls_ = *msg;
  }

  /// \brief Manages TF tree and publishes synchronized visualization data.
  /// \param stamp - hardware timestamp from encoders.
  void publish_data(const rclcpp::Time & stamp)
  {
    ...
    // 2. Odom -> green/base_footprint TF
    ...
    // 3. SLAM Path (Corrected, anchored to map)
    ...
    // --- PURE ODOMETRY BRANCH (BLUE ROBOT) ---

    // 4. nusim/world -> blue/base_footprint TF
    // Directly linked to static world frame to ignore SLAM corrections.
    ...
    // 5. Blue Path (Pure drift, anchored to nusim/world)
    // Synchronized timestamp and frame locking prevents bouncing.
    ...
    // 6. Joint States for spinning wheels
  ...
  rclcpp::Subscription<visualization_msgs::msg::MarkerArray>::SharedPtr landmark_sub_, wall_sub_;
